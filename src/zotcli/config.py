...
def _write_config_path() -> Path:
    """Return the path to the zotcli-home config.toml (for write settings)."""
    from zotcli.paths import config_path
    return config_path()


def _load_write_config() -> dict:
    """Load the zotcli-home config.toml, merging with write defaults."""
    import copy
...
def _save_write_config_atomic(cfg: dict) -> None:
    """Write cfg to zotcli-home config.toml atomically (write-then-rename)."""
    import tempfile
    p = _write_config_path()
    p.parent.mkdir(parents=True, exist_ok=True)
...
def set_write_enabled(value: bool) -> None:
    """Persist write.enabled to <zotcli-home>/config.toml atomically."""
    cfg = _load_write_config()
