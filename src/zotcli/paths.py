"""Cross-platform self-contained path resolution for zotcli home directory.

Resolution order (per PLAN_WRITE.md §7.1):
1. ZOTCLI_HOME env var, if set.
2. Walk up from this file's directory looking for a sibling SKILL.md →
   use <that-dir>/.zotcli/.
3. Final fallback: Path.home() / ".zotcli".
...
def zotcli_home() -> Path:
    """Return the zotcli home directory (does NOT create it).

    Resolution order:
    1. ZOTCLI_HOME env var.
    2. Sibling SKILL.md search → <skill-root>/.zotcli/
    3. Path.home() / ".zotcli"
    """
    env_home = os.environ.get("ZOTCLI_HOME")
    if env_home:
        return Path(env_home)

    skill_root = _find_skill_root()
    if skill_root is not None:
        return skill_root / ".zotcli"

    return Path.home() / ".zotcli"
...
def credentials_path() -> Path:
    ...
    p = zotcli_home() / "credentials.json"
    _ensure(p.parent)
    return p
...
def sessions_path() -> Path:
    ...
    p = zotcli_home() / "cache" / "sessions.jsonl"
    _ensure(p.parent)
    return p
