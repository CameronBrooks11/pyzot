"""Unit tests for zotcli.write.find_file (resolver pipeline)."""
...
def test_build_resolvers_empty_inputs():
    from zotcli.write.find_file import build_resolvers
...
def test_build_resolvers_doi_first():
    from zotcli.write.find_file import build_resolvers
    with patch("zotcli.write.oa_search.search_oa", return_value=[]):
...
def test_build_resolvers_oa_caps_at_max():
    from zotcli.write.find_file import _MAX_RESOLVERS, build_resolvers
    from zotcli.write.oa_search import OAResult

    many = [OAResult(url=f"https://x/{i}.pdf", page_url=None, version=None) for i in range(20)]
    with patch("zotcli.write.oa_search.search_oa", return_value=many):
...
def test_extract_pdf_url_meta_tag():
    from zotcli.write.find_file import _extract_pdf_url
...
def test_find_file_returns_first_successful_download(tmp_path):
    ...
    from zotcli.write.find_file import FindFileResult, find_file
    from zotcli.write.oa_search import OAResult

    fake_pdf = b"%PDF-1.4 fake content"

    with (
        patch("zotcli.write.oa_search.search_oa", return_value=[
            OAResult(url="https://x/pdf", page_url=None, version="publishedVersion"),
        ]),
        patch("zotcli.write.find_file._http_get_pdf", return_value=fake_pdf),
    ):
