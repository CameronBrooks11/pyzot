...
def test_extract_pdf_url_href_pdf():
    ...
    assert (
        _extract_pdf_url(html, "https://base.org/article/123")
        == "https://base.org/download/paper.pdf?token=x"
    )
...
def test_find_file_returns_first_successful_download(tmp_path):
    ...
    with (
        patch(
            "pyzot.write.find_file._scrape_pdf_url_from_page", return_value="https://x/paper.pdf"
        ),
