...
def isolated_config(monkeypatch, tmp_path):
    ...
    # Patch zotcli_home to return tmp_path / ".zotcli"
    fake_home = tmp_path / ".zotcli"

    import zotcli.paths as paths_mod
    monkeypatch.setattr(paths_mod, "zotcli_home", lambda: fake_home)

    # Also patch in config.py's reference to paths
    import zotcli.config as cfg_mod
    monkeypatch.setattr(
        cfg_mod,
        "_write_config_path",
...
def test_unpaywall_email_default_none():
    ...
    from zotcli.config import get_unpaywall_email
...
def test_set_and_get_write_enabled_true():
    ...
    from zotcli.config import set_write_enabled, get_write_enabled
...
def test_round_trip_persists_to_file(isolated_config):
    ...
    from zotcli.config import set_write_enabled
    set_write_enabled(True)
...
def test_set_get_config_value_string():
    ...
    from zotcli.config import set_config_value, get_config_value
...
def test_set_unpaywall_email():
    ...
    from zotcli.config import set_config_value, get_unpaywall_email
